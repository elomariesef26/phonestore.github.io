...
async function loadAll() {
  try {
    const [p, cat, c, s, u, pay, log, t, sess, sup, pur, inv] = await Promise.all([
...
function renderProducts() {
  ...
  const filtered = state.products.filter(p => {
    ...
    return !q || name.includes(q) || brand.includes(q) || sku.includes(q);
  });
  ...
  <div class="modal-overlay" role="dialog" aria-modal="true" id="productModalOverlay">
    <div class="modal">
      ...
      <div class="field-row">
        ...
        <div class="field"><label>Code P / code produit</label><input id="f_sku" value="${esc(p.sku)}" placeholder="APL-IP14-128"></div>
      </div>
...
function renderSettings() {
  const admin = isAdmin();
  const tabs = admin
    ? [{ id: 'profil', label: 'Mon profil' }, { id: 'vendeurs', label: 'Comptes vendeurs' }, { id: 'categories', label: 'Catégories' }, { id: 'historique', label: "Historique d'opérations" }, { id: 'sauvegarde', label: 'Sauvegarde' }, { id: 'inventaire', label: 'Inventaire physique' }, { id: 'reports', label: 'Rapports' }]
...
async function restoreFromFile(file) {
  ...
  try {
    ...
    const results = await Promise.all([
      ...
      saveOplog(), saveSuppliers(), savePurchases(), saveInvoices(), safeSet('shop:theme-pref', state.theme, false),
    ]);
...
function bindEvents() {
  ...
  if (saveProductModal) saveProductModal.addEventListener('click', async () => {
    ...
    if (sku && duplicateSku) { toast('Ce SKU est déjà utilisé par un autre produit'); return; }
    ...
    if (isNewProduct) {
      ...
      if (initQty > 0) {
        ...
        const purchase = {
          id: uid('pu'), date: localIsoFromParts(purchaseDateStr, localTimeInputValue()), supplierId,
