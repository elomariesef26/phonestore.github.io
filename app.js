        ...
        <tbody>
          ${sorted.map(s => {
            ...
            <tr>
              ...
              <td data-label="Total" class="price">${money(s.total)}</td>
              <td data-label=""><div class="row-actions"><button class="btn btn-sm" data-print-sale="${s.id}" title="Imprimer le reçu" aria-label="Imprimer le reçu">${ICONS.printer}</button><button class="btn btn-sm" data-print-delivery="${s.id}" title="Bon de livraison">${ICONS.printer}BL</button><button class="btn btn-sm" data-pdf-delivery="${s.id}" title="Télécharger le PDF">PDF</button><button class="btn btn-sm" data-share-delivery-whatsapp="${s.id}" title="Envoyer par WhatsApp">WA</button><button class="btn btn-sm" data-share-delivery-email="${s.id}" title="Envoyer par e-mail">E-mail</button>${isAdmin() ? `<button class="btn btn-sm icon-btn" data-edit-sale="${s.id}" title="Modifier la vente" aria-label="Modifier la vente">${ICONS.edit}</button><button class="btn btn-sm btn-danger icon-btn" data-delete-sale="${s.id}" title="Supprimer la vente" aria-label="Supprimer la vente">${ICONS.trash}</button>` : ''}</div></td>
            </tr>`;
          }).join('')}
        </tbody>
      </table>`}
...
function bindEvents() {
  ...
  const pfBon = document.getElementById('pf_bon'); if(pfBon) pfBon.addEventListener('input',e=>{state.purchaseFilters.bonNumber=e.target.value;refreshContent();});
  ...
  if (saveProductModal) saveProductModal.addEventListener('click', async () => {
    ...
    if (isNewProduct) {
      ...
      if (quickAddLineId && state.purchaseDraftLines) {
        ...
        if (purchaseLine) { purchaseLine.productId = newProduct.id; purchaseLine.sku = newProduct.sku; purchaseLine.unitPrice = document.getElementById('f_sell')?.value || ''; }
