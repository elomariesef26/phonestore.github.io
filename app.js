...
function dashboardSalesSummary(sales) {
  ...
  const historicalSaleCosts = calculateHistoricalSaleUnitCosts();
  sales.forEach(sale => {
    ...
    items.forEach((item, itemIndex) => {
      ...
      const unitCost = historicalSaleCosts.get(`${sale.id}:${(sale.items || []).indexOf(item)}`) ?? (Number(item.unitCost) || 0);
      units += qty; margin += gross - allocatedDiscount - (unitCost * qty);
...
function calculateHistoricalSaleUnitCosts() {
  const costs = new Map(); const lotsByProduct = new Map();
  state.purchases.forEach((purchase, purchaseIndex) => (purchase.items || []).forEach((item, itemIndex) => {
    if (!item.productId) return;
    const lots = lotsByProduct.get(item.productId) || [];
    lots.push({ date: purchase.date, purchaseIndex, itemIndex, remaining: Math.max(0, Number(item.qty) || 0), unitCost: Number(item.unitCost) || 0 });
    lotsByProduct.set(item.productId, lots);
  }));
  lotsByProduct.forEach(lots => lots.sort((a, b) => new Date(a.date) - new Date(b.date) || a.purchaseIndex - b.purchaseIndex || a.itemIndex - b.itemIndex));
  [...state.sales].sort((a, b) => new Date(a.date) - new Date(b.date)).forEach(sale => (sale.items || []).forEach((item, itemIndex) => {
    if (item.isService || item.serviceId || item.isRecharge || !item.productId) return;
    const qty = Math.max(0, Number(item.qty) || 0); let left = qty; let total = 0;
    for (const lot of (lotsByProduct.get(item.productId) || [])) {
      if (left <= 0) break;
      const take = Math.min(lot.remaining, left); if (take <= 0) continue;
      lot.remaining -= take; left -= take; total += take * lot.unitCost;
    }
    if (qty > 0 && left <= 0) costs.set(`${sale.id}:${itemIndex}`, total / qty);
  }));
  return costs;
}
function buildMarginReportData() {
  ...
  sales.forEach(sale => {
    ...
    items.forEach(item => {
      const qty = Number(item.qty) || 0; const gross = (Number(item.price) || 0) * qty;
      const cost = (historicalSaleCosts.get(`${sale.id}:${(sale.items || []).indexOf(item)}`) ?? (Number(item.unitCost) || 0)) * qty;
      const allocatedDiscount = grossSubtotal > 0 ? saleDiscount * gross / grossSubtotal : 0;
  ...
  const detailRows = sales.map(sale => {
    const cost = (sale.items || []).reduce((sum, it, itemIndex) => sum + (Number(it.qty) || 0) * (historicalSaleCosts.get(`${sale.id}:${itemIndex}`) ?? (Number(it.unitCost) || 0)), 0);
...
function buildOperationsReportData() {
  ...
  const historicalSaleCosts = new Map();
  const reportLots = new Map();
  state.purchases.forEach((purchase, purchaseIndex) => (purchase.items || []).forEach((item, itemIndex) => {
    if (!item.productId) return;
    const entries = reportLots.get(item.productId) || [];
    entries.push({ date: purchase.date, purchaseIndex, itemIndex, qty: Math.max(0, Number(item.qty) || 0), remaining: Math.max(0, Number(item.qty) || 0), unitCost: Number(item.unitCost) || 0 });
    reportLots.set(item.productId, entries);
  }));
  reportLots.forEach(entries => entries.sort((a, b) => new Date(a.date) - new Date(b.date) || a.purchaseIndex - b.purchaseIndex || a.itemIndex - b.itemIndex));
  [...state.sales].sort((a, b) => new Date(a.date) - new Date(b.date)).forEach(sale => (sale.items || []).forEach((item, itemIndex) => {
    if (item.isService || item.serviceId || item.isRecharge || !item.productId) return;
    const requested = Math.max(0, Number(item.qty) || 0); let left = requested; let totalCost = 0;
    for (const lot of (reportLots.get(item.productId) || [])) {
      if (left <= 0) break;
      const take = Math.min(lot.remaining, left); if (take <= 0) continue;
      lot.remaining -= take; left -= take; totalCost += take * lot.unitCost;
    }
    if (requested > 0 && left <= 0) historicalSaleCosts.set(`${sale.id}:${itemIndex}`, totalCost / requested);
  }));
  sales.forEach(sale => {
    const items = (sale.items || []).filter(item => Number(item.qty) > 0);
    ...
    includedItems.forEach((item, itemIndex) => {
      const product = state.products.find(p => p.id === item.productId);
      const qty = Number(item.qty) || 0; const buy = historicalSaleCosts.get(`${sale.id}:${(sale.items || []).indexOf(item)}`) ?? (Number(item.unitCost) || 0); const sell = Number(item.price) || 0;
...
function buildPeriodFinancialSummary(range, stockData) {
  ...
  state.sales.filter(inRange).forEach(sale => {
    ...
    items.forEach(item => {
      ...
      if (item.isRecharge) {
        ...
        summary.productUnits += qty; summary.productSales += gross; summary.productCost += cost; summary.productDiscount += allocatedDiscount; summary.productMargin += margin;
      }
    });
  });
  ...
  // Product sales are reported gross; discounts must be deducted once from
  // the gross revenue before subtracting the FIFO cost.
  summary.productMargin = summary.productSales - summary.productDiscount - summary.productCost;
...
function exportMarginReportXLSX() {
  ...
  const safeName = name => String(name || 'Données').replace(/[\\/*?:\[\]]/g, ' ').trim().slice(0, 31) || 'Données';
  const addFormulaSheet = (name, headers, rows, widths, formulas = {}, totalFormulas = {}) => {
    const sheetName = safeName(name); const ws = XLSX.utils.json_to_sheet(rows || [], { header: headers });
    headers.forEach((header, index) => { ws[XLSX.utils.encode_cell({ r: 0, c: index })] = { t: 's', v: header }; });
    const endRow = Math.max(rows.length + 1, 2);
    rows.forEach((row, rowIndex) => Object.entries(formulas).forEach(([column, formula]) => {
      const columnIndex = headers.indexOf(column); if (columnIndex < 0) return;
      const cell = XLSX.utils.encode_cell({ r: rowIndex + 1, c: columnIndex }); ws[cell] = { t: 'n', f: formula(rowIndex + 2), v: Number(row[column]) || 0 };
    }));
    if (rows.length && Object.keys(totalFormulas).length) {
      const totalRow = endRow + 1; ws[XLSX.utils.encode_cell({ r: totalRow - 1, c: 0 })] = { t: 's', v: 'TOTAL' };
      Object.entries(totalFormulas).forEach(([column, formula]) => {
        const columnIndex = headers.indexOf(column); if (columnIndex < 0) return;
        const cell = XLSX.utils.encode_cell({ r: totalRow - 1, c: columnIndex }); ws[cell] = { t: 'n', f: formula(endRow), v: rows.reduce((sum, row) => sum + (Number(row[column]) || 0), 0) };
      });
    }
    ws['!cols'] = widths.map(w => ({ wch: w })); XLSX.utils.book_append_sheet(wb, ws, sheetName);
    return { name: sheetName, endRow, totalRow: rows.length ? endRow + 1 : null };
  };
  const addIf = (key, name, headers, rows, widths, formulas, totals) => reportSectionEnabled(key) ? addFormulaSheet(name, headers, rows, widths, formulas, totals) : null;
  const saleHeaders = ['Date', 'Référence', 'Produit', 'Code P', 'Catégorie', 'Quantité', 'Prix achat unitaire (DH)', 'Prix vente unitaire (DH)', 'Remise (DH)', 'Montant brut (DH)', 'Montant net (DH)', 'Coût total (DH)', 'Marge unitaire (DH)', 'Marge totale (DH)', 'Taux marge (%)', 'Client', 'Vendeur'];
  const saleRows = operations.saleRows.map(r => ({ Date: String(r.date || '').slice(0, 10), Référence: r.ref, Produit: r.product, 'Code P': r.sku, Catégorie: r.category, Quantité: r.qty, 'Prix achat unitaire (DH)': r.unitCost, 'Prix vente unitaire (DH)': r.unitPrice, 'Remise (DH)': r.discount, 'Montant brut (DH)': r.gross, 'Montant net (DH)': r.net, 'Coût total (DH)': r.cost, 'Marge unitaire (DH)': r.unitMargin, 'Marge totale (DH)': r.margin, 'Taux marge (%)': r.marginRate, Client: r.party, Vendeur: r.seller }));
  const salesSheet = addIf('sales', 'Détail ventes', saleHeaders, saleRows, [20, 15, 28, 18, 20, 12, 20, 22, 18, 18, 18, 18, 18, 18, 16, 24, 22], {
    'Montant brut (DH)': row => `F${row}*H${row}`, 'Montant net (DH)': row => `J${row}-I${row}`, 'Coût total (DH)': row => `F${row}*G${row}`, 'Marge unitaire (DH)': row => `H${row}-G${row}`, 'Marge totale (DH)': row => `K${row}-L${row}`, 'Taux marge (%)': row => `IFERROR(N${row}/K${row}*100,0)`,
  }, { 'Quantité': end => `SUM(F2:F${end})`, 'Remise (DH)': end => `SUM(I2:I${end})`, 'Montant brut (DH)': end => `SUM(J2:J${end})`, 'Montant net (DH)': end => `SUM(K2:K${end})`, 'Coût total (DH)': end => `SUM(L2:L${end})`, 'Marge totale (DH)': end => `SUM(N2:N${end})` });
  const purchaseHeaders = ['Date', 'Référence', 'Produit', 'SKU', 'Catégorie', 'Fournisseur', 'Quantité', 'Prix achat unitaire (DH)', 'Prix vente unitaire prévu (DH)', 'Total achat (DH)', 'Valeur vente prévue (DH)', 'Marge potentielle (DH)', 'Taux marge (%)'];
  const purchaseRows = operations.purchaseRows.map(r => ({ Date: fmtDateTime(r.date), Référence: r.ref, Produit: r.product, SKU: r.sku, Catégorie: r.category, Fournisseur: r.party, Quantité: r.qty, 'Prix achat unitaire (DH)': r.unitCost, 'Prix vente unitaire prévu (DH)': r.unitPrice, 'Total achat (DH)': r.totalPurchase, 'Valeur vente prévue (DH)': r.saleValue, 'Marge potentielle (DH)': r.potentialMargin, 'Taux marge (%)': r.marginRate }));
  const purchaseSheet = addIf('purchases', 'Détail achats', purchaseHeaders, purchaseRows, [20, 15, 28, 18, 20, 24, 12, 20, 25, 18, 24, 22, 16], {
    'Total achat (DH)': row => `G${row}*H${row}`, 'Valeur vente prévue (DH)': row => `G${row}*I${row}`, 'Marge potentielle (DH)': row => `K${row}-J${row}`, 'Taux marge (%)': row => `IFERROR(L${row}/K${row}*100,0)`,
  }, { 'Quantité': end => `SUM(G2:G${end})`, 'Total achat (DH)': end => `SUM(J2:J${end})`, 'Valeur vente prévue (DH)': end => `SUM(K2:K${end})`, 'Marge potentielle (DH)': end => `SUM(L2:L${end})` });
  const paymentRows = operations.paymentRows.map(r => ({ Date: fmtDateTime(r.date), Référence: r.ref, Client: r.party, Note: r.note, 'Montant (DH)': r.amount }));
  const paymentsSheet = addIf('payments', 'Paiements', ['Date', 'Référence', 'Client', 'Note', 'Montant (DH)'], paymentRows, [20, 18, 26, 40, 18], {}, { 'Montant (DH)': end => `SUM(E2:E${end})` });
  const purchaseBonRows = buildPurchaseBonSummary(operations.range, !!f.includeEquipmentInPurchaseBons).map(r => ({ 'N Bon': r.bonNumber, Date: fmtDateTime(r.date), Fournisseur: r.supplier || 'Non spécifié', 'Coût total (DH)': r.total }));
  const bonsSheet = addIf('purchaseBons', 'Synthèse bons', ['N Bon', 'Date', 'Fournisseur', 'Coût total (DH)'], purchaseBonRows, [18, 20, 28, 20], {}, { 'Coût total (DH)': end => `SUM(D2:D${end})` });
  ...
  const stockSheet = addIf('stock', 'Situation stock', ['Produit', 'Catégorie', 'SKU', 'Stock actuel', 'Lots', 'Valeur prix achat (DH)', 'Valeur prix vente (DH)', 'Marge potentielle (DH)'], stockRows, [28, 24, 20, 14, 10, 22, 22, 24], {}, { 'Stock actuel': end => `SUM(D2:D${end})`, 'Valeur prix achat (DH)': end => `SUM(F2:F${end})`, 'Valeur prix vente (DH)': end => `SUM(G2:G${end})`, 'Marge potentielle (DH)': end => `SUM(H2:H${end})` });
  const salesRef = salesSheet ? `'${salesSheet.name}'` : "'Détail ventes'"; const purchaseRef = purchaseSheet ? `'${purchaseSheet.name}'` : "'Détail achats'"; const paymentsRef = paymentsSheet ? `'${paymentsSheet.name}'` : "'Paiements'";
  const salesEnd = salesSheet?.endRow || 1; const purchaseEnd = purchaseSheet?.endRow || 1; const paymentsEnd = paymentsSheet?.endRow || 1;
  const summaryRows = [{ Période: `${f.from || 'Début'} → ${f.to || 'Aujourd’hui'}`, Ventes: data.sales.length, Unités: data.units, 'Remises (DH)': data.discounts, 'CA net (DH)': data.revenue, 'Coût FIFO (DH)': data.totalCost, 'Marge (DH)': data.margin, 'Taux marge (%)': data.marginRate }];
  addIf('summary', 'Synthèse', ['Période', 'Ventes', 'Unités', 'Remises (DH)', 'CA net (DH)', 'Coût FIFO (DH)', 'Marge (DH)', 'Taux marge (%)'], summaryRows, [26, 14, 12, 16, 16, 16, 16, 16], { Ventes: () => `COUNTA(${salesRef}!A2:A${salesEnd})`, Unités: () => `SUM(${salesRef}!F2:F${salesEnd})`, 'Remises (DH)': () => `SUM(${salesRef}!I2:I${salesEnd})`, 'CA net (DH)': () => `SUM(${salesRef}!K2:K${salesEnd})`, 'Coût FIFO (DH)': () => `SUM(${salesRef}!L2:L${salesEnd})`, 'Marge (DH)': () => `SUM(${salesRef}!N2:N${salesEnd})`, 'Taux marge (%)': () => `IFERROR(G2/E2*100,0)` });
  addIf('stats', 'Synthèse opérations', ['Période', 'Total ventes (DH)', 'Total achats (DH)', 'Total remises (DH)', 'Coût ventes (DH)', 'Marge ventes (DH)', 'Net ventes - achats (DH)', 'Paiements (DH)'], [{ Période: `${operations.range.from} → ${operations.range.to}`, 'Total ventes (DH)': operations.salesAmount, 'Total achats (DH)': operations.purchasesAmount, 'Total remises (DH)': operations.salesDiscount, 'Coût ventes (DH)': operations.salesCost, 'Marge ventes (DH)': operations.salesMargin, 'Net ventes - achats (DH)': operations.net, 'Paiements (DH)': operations.paymentsAmount }], [24, 18, 18, 18, 18, 18, 22, 18], { 'Total ventes (DH)': () => `SUM(${salesRef}!K2:K${salesEnd})`, 'Total achats (DH)': () => `SUM(${purchaseRef}!J2:J${purchaseEnd})`, 'Total remises (DH)': () => `SUM(${salesRef}!I2:I${salesEnd})`, 'Coût ventes (DH)': () => `SUM(${salesRef}!L2:L${salesEnd})`, 'Marge ventes (DH)': () => `SUM(${salesRef}!N2:N${salesEnd})`, 'Net ventes - achats (DH)': () => `B2-C2`, 'Paiements (DH)': () => `SUM(${paymentsRef}!E2:E${paymentsEnd})` });
  const financialRows = [{ Période: `${operations.range.from} → ${operations.range.to}`, 'Capital configuré (DH)': financial.capitalAmount, 'Solde recharges (DH)': financial.rechargeBalance, 'Reste du capital (DH)': financial.remainingCapital, 'Produits vendus (unités)': financial.productUnits, 'Ventes produits (DH)': financial.productSales, 'Prix achat produits vendus (DH)': financial.productCost, 'Remises produits (DH)': financial.productDiscount, 'Marge produits (DH)': financial.productMargin, 'Recharges vendues (DH)': financial.rechargeSales, 'Marge recharges (DH)': financial.rechargeMargin, 'Services vendus (DH)': financial.serviceSales, 'Marge services (DH)': financial.serviceMargin, 'Dépenses courantes (DH)': financial.currentExpenses, 'Bénéfice estimé (DH)': financial.estimatedProfit, 'Achats stock (DH)': financial.stockPurchases, 'Équipements (DH)': financial.equipmentPurchases, 'Stock restant au prix achat (DH)': financial.stockRemainingCost }];
  addIf('summary', 'Synthèse financière', Object.keys(financialRows[0]), financialRows, [24, 18, 18, 22, 18, 18, 18, 18, 18, 18, 20, 18, 18, 18, 24], { 'Produits vendus (unités)': () => `SUM(${salesRef}!F2:F${salesEnd})`, 'Ventes produits (DH)': () => `SUM(${salesRef}!K2:K${salesEnd})`, 'Prix achat produits vendus (DH)': () => `SUM(${salesRef}!L2:L${salesEnd})`, 'Remises produits (DH)': () => `SUM(${salesRef}!I2:I${salesEnd})`, 'Marge produits (DH)': () => `F2-G2-H2`, 'Bénéfice estimé (DH)': () => `I2+K2+M2-N2`, 'Reste du capital (DH)': () => `B2-(R2+G2+Q2+C2+J2)` });
  const productRows = data.productRows.map(r => ({ 'Code P': r.sku, Produit: r.product, Unités: r.units, 'Remises allouées (DH)': r.discount, 'CA net (DH)': r.net, 'Coût FIFO (DH)': r.cost, 'Marge (DH)': r.margin, 'Taux marge (%)': r.marginRate }));
  addIf('productPerformance', 'Par produit', ['Code P', 'Produit', 'Unités', 'Remises allouées (DH)', 'CA net (DH)', 'Coût FIFO (DH)', 'Marge (DH)', 'Taux marge (%)'], productRows, [20, 32, 12, 16, 20, 16, 16, 16], { Unités: row => `SUMIF(${salesRef}!C:C,B${row},${salesRef}!F:F)`, 'Remises allouées (DH)': row => `SUMIF(${salesRef}!C:C,B${row},${salesRef}!I:I)`, 'CA net (DH)': row => `SUMIF(${salesRef}!C:C,B${row},${salesRef}!K:K)`, 'Coût FIFO (DH)': row => `SUMIF(${salesRef}!C:C,B${row},${salesRef}!L:L)`, 'Marge (DH)': row => `E${row}-F${row}`, 'Taux marge (%)': row => `IFERROR(G${row}/E${row}*100,0)` }, { Unités: end => `SUM(C2:C${end})`, 'Remises allouées (DH)': end => `SUM(D2:D${end})`, 'CA net (DH)': end => `SUM(E2:E${end})`, 'Coût FIFO (DH)': end => `SUM(F2:F${end})`, 'Marge (DH)': end => `SUM(G2:G${end})` });
  const dayRows = data.dayRows.map(r => ({ Date: r.date, Ventes: r.sales, Unités: r.units, 'CA net (DH)': r.revenue, 'Coût FIFO (DH)': r.cost, 'Marge (DH)': r.margin, 'Taux marge (%)': r.marginRate }));
  addIf('dailyEvolution', 'Par jour', ['Date', 'Ventes', 'Unités', 'CA net (DH)', 'Coût FIFO (DH)', 'Marge (DH)', 'Taux marge (%)'], dayRows, [16, 12, 12, 16, 16, 16, 16], { Ventes: row => `COUNTIF(${salesRef}!A:A,A${row})`, Unités: row => `SUMIF(${salesRef}!A:A,A${row},${salesRef}!F:F)`, 'CA net (DH)': row => `SUMIF(${salesRef}!A:A,A${row},${salesRef}!K:K)`, 'Coût FIFO (DH)': row => `SUMIF(${salesRef}!A:A,A${row},${salesRef}!L:L)`, 'Marge (DH)': row => `D${row}-E${row}`, 'Taux marge (%)': row => `IFERROR(F${row}/D${row}*100,0)` }, { Ventes: end => `SUM(B2:B${end})`, Unités: end => `SUM(C2:C${end})`, 'CA net (DH)': end => `SUM(D2:D${end})`, 'Coût FIFO (DH)': end => `SUM(E2:E${end})`, 'Marge (DH)': end => `SUM(F2:F${end})` });
  XLSX.writeFile(wb, `phonestock-rapport-marges-${new Date().toISOString().slice(0, 10)}.xlsx`); toast('Rapport Excel exporté avec formules et liens entre feuilles');
}
...
function excelDate(value, fallback = new Date().toISOString()) {
  if (value instanceof Date && !Number.isNaN(value.getTime())) {
    return localIsoFromParts(localDateInputValue(value), localTimeInputValue(value));
  }
  if (typeof value === 'number' && XLSX && XLSX.SSF) {
    ...
    if (d) {
      ...
      return localIsoFromParts(date, time);
    }
  }
  const raw = String(value || '').trim();
  const fr = raw.match(/^(\d{1,2})[\/-](\d{1,2})[\/-](\d{4})(?:[ T](\d{1,2}):(\d{2})(?::(\d{2}))?)?$/);
  if (fr) {
    const date = `${fr[3]}-${String(fr[2]).padStart(2, '0')}-${String(fr[1]).padStart(2, '0')}`;
    const time = `${String(fr[4] || 0).padStart(2, '0')}:${fr[5] || '00'}`;
    return localIsoFromParts(date, time);
  }
  const isoLocal = raw.match(/^(\d{4})-(\d{2})-(\d{2})(?:[ T](\d{1,2}):(\d{2})(?::(\d{2}))?)?$/);
  if (isoLocal) {
    const time = `${String(isoLocal[4] || 0).padStart(2, '0')}:${isoLocal[5] || '00'}`;
    return localIsoFromParts(`${isoLocal[1]}-${isoLocal[2]}-${isoLocal[3]}`, time);
  }
  const parsed = value ? new Date(value) : null;
  return parsed && !Number.isNaN(parsed.getTime()) ? localIsoFromParts(localDateInputValue(parsed), localTimeInputValue(parsed)) : fallback;
}
...
function downloadExcelRows(rows, filename, sheetName) {
  ...
  const safeSheetName = String(sheetName || 'Données').replace(/[\\/*?:\[\]]/g, ' ').trim().slice(0, 31) || 'Données';
  excel.utils.book_append_sheet(wb, ws, safeSheetName);
...
function exportHistoryXLSX() {
  const sales = filteredSalesForHistoryExport();
  const fifoCosts = new Map(); const remainingByLot = new Map(); const lotsByProduct = new Map();
  state.purchases.forEach((purchase, purchaseIndex) => (purchase.items || []).forEach((item, itemIndex) => {
    if (!item.productId) return;
    const entries = lotsByProduct.get(item.productId) || []; entries.push({ purchaseIndex, itemIndex, date: purchase.date, item }); lotsByProduct.set(item.productId, entries);
    remainingByLot.set(`${purchaseIndex}:${itemIndex}`, Math.max(0, Number(item.qty) || 0));
  }));
  lotsByProduct.forEach(entries => entries.sort((a, b) => new Date(a.date) - new Date(b.date)));
  [...state.sales].sort((a, b) => new Date(a.date) - new Date(b.date)).forEach(sale => (sale.items || []).forEach((item, itemIndex) => {
    if (item.isService || item.serviceId || item.isRecharge || !item.productId) return;
    let left = Math.max(0, Number(item.qty) || 0); let cost = 0;
    for (const lot of (lotsByProduct.get(item.productId) || [])) {
      const key = `${lot.purchaseIndex}:${lot.itemIndex}`; const available = remainingByLot.get(key) || 0; const take = Math.min(available, left);
      if (take > 0) { cost += take * (Number(lot.item.unitCost) || 0); remainingByLot.set(key, available - take); left -= take; }
      if (left <= 0) break;
    }
    if (left <= 0 && Number(item.qty) > 0) fifoCosts.set(`${sale.id}:${itemIndex}`, cost / Number(item.qty));
  }));
  const rows = []; sales.forEach(s => { const c = state.clients.find(x => x.id === s.clientId); (s.items || []).forEach((it, itemIndex) => { const p = state.products.find(x => x.id === it.productId); const unitCost = fifoCosts.get(`${s.id}:${itemIndex}`) ?? (Number(it.unitCost) || 0);
    rows.push({ 'ID vente': s.id, 'N° vente': s.number || '', Date: fmtDateTime(s.date), 'Code client': c ? c.clientCode : '', Client: c ? c.name : 'Client de passage', Vendeur: s.sellerName || '', Paiement: s.payment || '', Statut: saleStatus(s), SKU: p ? p.sku : '', Produit: it.name || (p && p.name) || '', Quantité: Number(it.qty) || 0, 'Prix unitaire (DH)': Number(it.price) || 0, 'Coût unitaire (DH)': unitCost, 'Sous-total': Number(s.subtotal) || 0, 'Remise (DH)': Number(s.discount) || 0, Total: Number(s.total) || 0, 'Payé (DH)': Number(s.paidAmount) || 0, 'Coût total (DH)': 0, 'Marge unitaire (DH)': 0, 'Marge totale (DH)': 0 });
  }); });
  const headers = ['ID vente', 'N° vente', 'Date', 'Code client', 'Client', 'Vendeur', 'Paiement', 'Statut', 'SKU', 'Produit', 'Quantité', 'Prix unitaire (DH)', 'Coût unitaire (DH)', 'Sous-total', 'Remise (DH)', 'Total', 'Payé (DH)', 'Coût total (DH)', 'Marge unitaire (DH)', 'Marge totale (DH)'];
  const ws = XLSX.utils.json_to_sheet(rows, { header: headers });
  headers.forEach((header, index) => { ws[XLSX.utils.encode_cell({ r: 0, c: index })] = { t: 's', v: header }; });
  rows.forEach((row, index) => { const r = index + 2; ws[`R${r}`] = { t: 'n', f: `K${r}*M${r}`, v: Number(row['Quantité']) * Number(row['Coût unitaire (DH)']) }; ws[`S${r}`] = { t: 'n', f: `L${r}-M${r}`, v: Number(row['Prix unitaire (DH)']) - Number(row['Coût unitaire (DH)']) }; ws[`T${r}`] = { t: 'n', f: `R${r}*S${r}`, v: Number(row['Quantité']) * (Number(row['Prix unitaire (DH)']) - Number(row['Coût unitaire (DH)'])) }; });
  ws['!cols'] = [18, 10, 20, 14, 24, 18, 14, 14, 20, 28, 10, 18, 18, 16, 14, 16, 16, 18, 20, 18].map(w => ({ wch: w }));
  const wb = XLSX.utils.book_new(); XLSX.utils.book_append_sheet(wb, ws, 'Historique ventes'); XLSX.writeFile(wb, `phonestock-historique-ventes-${new Date().toISOString().slice(0, 10)}.xlsx`);
}
async function importHistoryXLSX(file) {
  try {
    ...
    const saleFingerprint = sale => {
      const items = (sale.items || []).map(item => `${item.productId || ''}:${Number(item.qty) || 0}:${Number(item.price) || 0}`).sort().join('|');
      return `${Number(sale.number) || ''}|${new Date(sale.date).getTime() || ''}|${items}|${Number(sale.total) || 0}`;
    };
    const existingSalesByNumber = new Map(state.sales.filter(sale => Number.isInteger(Number(sale.number))).map(sale => [Number(sale.number), sale]));
    const imported = []; let number = nextSaleNumber(); let skippedDuplicates = 0;
    for (const [, group] of groups) { const first = group[0]; const client = findClientByImport(first['Code client'], first.Client); const items = group.map(r => { const p = findProductBySku(r.SKU); return { productId: p.id, name: p.name, qty: excelNumber(r.Quantité), price: excelNumber(r['Prix unitaire (DH)'], 0), unitCost: excelNumber(r['Coût unitaire (DH)'], 0) }; });
      ...
      const sourceNumber = Number(first['N° vente']);
      const candidate = { id: uid('s'), number: number++, date: excelDate(first.Date), items, subtotal, discount, total, payment: String(first.Paiement || 'Espèces'), clientId: client ? client.id : null, paidAmount: Math.max(0, Math.min(total, excelNumber(first['Payé (DH)'], 0))), sellerId: null, sellerName: String(first.Vendeur || 'Import Excel'), source: 'Import Excel' };
      const existing = Number.isInteger(sourceNumber) ? existingSalesByNumber.get(sourceNumber) : null;
      if (existing && saleFingerprint(existing) === saleFingerprint({ ...candidate, number: sourceNumber })) { skippedDuplicates += 1; continue; }
      imported.push(candidate);
    }
    if (!imported.length) { toast(skippedDuplicates ? `${skippedDuplicates} vente(s) déjà présentes : aucun doublon importé.` : 'Aucune vente valide à importer.'); return; }
    const importedItems = imported.flatMap(sale => sale.items || []);
    if (!confirmLocalized(`Importer ${imported.length} vente(s) historique(s) et diminuer le stock ? Les paiements ne seront pas modifiés.`)) return;
    const consumedItems = [];
    for (const item of importedItems) {
      const fifoCost = consumeStockFIFO(item.productId, item.qty);
      if (fifoCost === null) {
        restoreStockForSaleItems(consumedItems);
        toast(`Stock insuffisant pour importer les ventes (${item.name || item.productId}).`);
        return;
      }
      // consumeStockFIFO returns the weighted average unit cost already.
      // Do not divide it by the quantity a second time.
      item.unitCost = fifoCost;
      consumedItems.push(item);
    }
    state.sales.push(...imported);
    const [salesResult, purchasesResult] = await Promise.all([saveSales(), savePurchases()]);
    if (salesResult === null || purchasesResult === null) {
      state.sales.splice(state.sales.length - imported.length, imported.length);
      restoreStockForSaleItems(consumedItems);
      await Promise.all([saveSales(), savePurchases()]);
      toast('Échec de sauvegarde de l’historique importé et du stock.');
      return;
    }
    await logOperation('Import Excel historique', `${imported.length} vente(s) avec stock diminué`); toast('Historique importé et stock mis à jour'); render();
