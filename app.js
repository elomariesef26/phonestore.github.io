...
// with the link). When running standalone (the downloaded ZIP, opened via
// file:// or hosted elsewhere), window.storage doesn't exist, so we fall
// back to the browser's own localStorage — this keeps every change saved
// on that device/browser even without Claude.ai's shared backend.
...
async function loadAll() {
  try {
    ...
    state.purchases = parseOrDefault(pur, []);
    ...
    console.info('[PhoneStock] Stockage :', hasNativeStorage() ? 'Claude.ai (partagé)' : 'navigateur local (localStorage) — origine : ' + location.origin,
...
function storageDiagnosticLine() {
  const mode = hasNativeStorage() ? 'Claude.ai (stockage partagé)' : 'navigateur local — ' + location.origin;
