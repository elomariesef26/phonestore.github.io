...
const state = {
  view: 'dashboard',
  theme: 'dark',
  branding: { name: 'PhoneStock', subtitle: 'Gestion boutique', icon: '' },
  loaded: false,
  products: [],
...
async function loadAll() {
  try {
    const [p, cat, c, s, u, pay, log, t, brandingStored, sess, sup, pur, inv] = await Promise.all([
  ...
  <div class="modal-overlay" role="dialog" aria-modal="true" id="productModalOverlay">
    <div class="modal">
      ...
      <div class="field-row">
        ...
        <div class="field"><label>Code P / code produit</label><input id="f_sku" value="${esc(p.sku)}" placeholder="Généré automatiquement si vide"><div class="muted" style="font-size:10.5px;margin-top:4px;">Si vous laissez ce champ vide, un Code P unique sera créé à partir du nom, de la marque et du modèle.</div></div>
      </div>
        ...
        <tbody>
          ${sorted.map(pu => {
            ...
            <tr>
              ...
              <td data-label="Actions" class="row-actions">${`<button type="button" class="btn btn-sm icon-btn" data-print-purchase="${pu.id}" title="Imprimer le bon d’achat" aria-label="Imprimer le bon d’achat">${ICONS.printer}</button>`}${canEdit ? `<button type="button" class="btn btn-sm icon-btn" data-edit-purchase="${pu.id}" title="Modifier l’achat" aria-label="Modifier l’achat">${ICONS.edit}</button>` : ''}${canDelete ? `<button type="button" class="btn btn-sm icon-btn btn-danger" data-delete-purchase="${pu.id}" title="Supprimer l’achat" aria-label="Supprimer l’achat">${ICONS.trash}</button>` : ''}</td>
            </tr>`;
          }).join('')}
        </tbody>
      </table>`}
        ...
        <div id="purchaseLines">
          ${lines.map(l => `
            <div class="purchase-line-grid">
              <div class="field"><label>Code P / code</label><div style="display:flex;gap:5px;"><input id="pl_sku_${l.id}" data-purchase-sku="${l.id}" placeholder="Code P ou code-barres" value="${esc(l.sku || '')}"><button type="button" class="btn btn-sm" data-scan-purchase-line="${l.id}" title="Scanner le code">Scanner</button></div></div>
...
function renderPOS() {
  ...
  const list = state.products.filter(p =>
    ...
    (!q || String(p.name || '').toLowerCase().includes(q) || String(p.brand || '').toLowerCase().includes(q) || String(p.sku || '').toLowerCase().includes(q))
  );
          ...
          <tbody>
            ${sales.map(s => {
              ...
              <tr ${isInvoiced ? 'style="opacity:.6;"' : ''}>
                ...
                <td data-label=""><button class="btn btn-sm" data-print-sale="${s.id}">${ICONS.printer}Bon</button><button class="btn btn-sm" data-print-delivery="${s.id}" title="Bon de livraison">${ICONS.printer}BL</button></td>
              </tr>
            `; }).join('')}
          </tbody>
        </table>
        ...
        <tbody>
          ${sorted.map(s => {
            ...
            <tr>
              ...
              <td data-label="Total" class="price">${money(s.total)}</td>
              <td data-label=""><button class="btn btn-sm" data-print-sale="${s.id}">${ICONS.printer}</button><button class="btn btn-sm" data-print-delivery="${s.id}" title="Bon de livraison">${ICONS.printer}BL</button></td>
            </tr>`;
          }).join('')}
        </tbody>
      </table>`}
...
function renderSettings() {
  const admin = isAdmin();
  const tabs = admin
    ? [{ id: 'profil', label: 'Mon profil' }, { id: 'identite', label: 'Identité de l’application' }, { id: 'vendeurs', label: 'Comptes vendeurs' }, { id: 'categories', label: 'Catégories' }, { id: 'historique', label: "Historique d'opérations" }, { id: 'sauvegarde', label: 'Sauvegarde' }, { id: 'inventaire', label: 'Inventaire physique' }, { id: 'reports', label: 'Rapports' }]
...
function renderSettingsIdentite() {
  ...
  return `<div class="card branding-settings" style="padding:20px;max-width:680px;">
    ...
    <div class="field"><label>Icône de l’application</label><input id="appBrandIcon" type="file" accept="image/png,image/jpeg,image/webp,image/svg+xml"><div class="branding-preview">${safeImageSrc(b.icon) ? `<img src="${esc(b.icon)}" alt="Aperçu de l’icône">` : '<span class="muted">Aucune icône personnalisée</span>'}</div></div>
...
function bindEvents() {
  ...
  if (saveBrandingBtn) saveBrandingBtn.addEventListener('click', async () => { const name=document.getElementById('appBrandName')?.value.trim(); const subtitle=document.getElementById('appBrandSubtitle')?.value.trim(); if (!name || !subtitle) { toast('Le nom et le sous-nom sont obligatoires.'); return; } state.branding={...state.branding,name,subtitle}; await safeSet('shop:branding',state.branding,false); await logOperation('Identité de l’application modifiée',name); toast('Identité enregistrée'); render(); });
  const resetBrandingBtn = document.getElementById('resetBrandingBtn');
  if (resetBrandingBtn) resetBrandingBtn.addEventListener('click', async () => { state.branding={name:'PhoneStock',subtitle:'Gestion boutique',icon:''}; await safeSet('shop:branding',state.branding,false); toast('Identité rétablie'); render(); });
  ...
  /* Print receipt */
    document.querySelectorAll('[data-print-delivery]').forEach(el => el.addEventListener('click', () => { const sale=state.sales.find(x=>x.id===el.dataset.printDelivery); if (sale) printHTML(buildDeliveryNoteHTML(sale)); }));
document.querySelectorAll('[data-print-sale]').forEach(el => el.addEventListener('click', () => {
  ...
  document.querySelectorAll('[data-print-purchase]').forEach(el => el.addEventListener('click', () => { const purchase=state.purchases.find(x=>x.id===el.dataset.printPurchase); if (purchase) printHTML(buildPurchaseOrderHTML(purchase)); }));
