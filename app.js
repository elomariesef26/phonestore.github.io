...
function getCategories() {
  return Array.from(new Set([...(state.categories || []), ...(state.products || []).map(product => product.category).filter(Boolean)]));
}
...
const state = {
  ...
  reportFilters: { from: '', to: '', sellerId: '', productId: '' },
...
async function saveProducts() { return safeSet('shop:products', JSON.stringify(state.products), true); }
async function saveCategories() { return safeSet('shop:categories', JSON.stringify(state.categories), true); }
async function saveClients() { return safeSet('shop:clients', JSON.stringify(state.clients), true); }
async function saveSales() { return safeSet('shop:sales', JSON.stringify(state.sales), true); }
async function saveUsers() { return safeSet('shop:users', JSON.stringify(state.users), true); }
async function savePayments() { return safeSet('shop:payments', JSON.stringify(state.payments), true); }
async function saveOplog() { return safeSet('shop:oplog', JSON.stringify(state.oplog), true); }
async function saveSuppliers() { return safeSet('shop:suppliers', JSON.stringify(state.suppliers), true); }
async function savePurchases() { return safeSet('shop:purchases', JSON.stringify(state.purchases), true); }
async function saveInvoices() { return safeSet('shop:invoices', JSON.stringify(state.invoices), true); }
...
async function loadAll() {
  try {
    const [p, cat, c, s, u, pay, log, t, brandingStored, sess, sup, pur, inv, generatedDocs] = await Promise.all([
      ...
      safeGet('shop:suppliers', true), safeGet('shop:purchases', true), safeGet('shop:invoices', true), safeGet('shop:generated-documents', true),
    ]);
...
function topbarAction() {
  ...
  if (state.view === 'pos') return `<input class="search-input topbar-search" id="posSearch" placeholder="Rechercher un produit ou Code P…" value="${esc(state.posSearch)}"><button class="btn" id="scanPosBtn">${ICONS.scan}Scanner</button>`;
...
function renderProductModal() {
  ...
  const p = isNew ? { name: '', brand: '', model: '', category: 'Smartphones', sku: '', lowStock: 3, photo: null, archived: false } : state.editingProduct;
      ...
      <div class="field-row">
        <div class="field"><label>Fournisseur (optionnel)</label>
          <select id="f_supplier">
            ...
            ${visibleSuppliers.map(s => `<option value="${s.id}">${esc(s.name)}</option>`).join('')}
          </select>
        </div>
        <div class="field"><label>Date d'achat</label><input id="f_purchase_date" type="date" value="${localDateInputValue()}"></div>
        <div class="field"><label>Heure d'achat</label><input id="f_purchase_time" type="time" value="${localTimeInputValue()}"></div>
      </div>
...
function renderPurchaseRegister() {
  ...
  if (purchaseQ) sorted=sorted.filter(pu=>[pu.id,pu.note,...(pu.items||[]).flatMap(i=>[i.name,i.sku])].some(v=>normalizeSearchText(v).includes(purchaseQ)));
  ...
  return `
    ...
    <div class="card">
      ...
      <table class="rcard">
        <thead><tr><th>Date</th><th>Fournisseur</th><th>Produits</th><th style="text-align:right;">Coût total</th><th>Actions</th></tr></thead>
        <tbody>
          ${sorted.map(pu => {
            ...
            <tr>
              ...
              <td data-label="Actions" class="row-actions">${`<button type="button" class="btn btn-sm icon-btn" data-print-purchase="${pu.id}" title="Imprimer le bon d’achat" aria-label="Imprimer le bon d’achat">${ICONS.printer}</button><button type="button" class="btn btn-sm icon-btn" data-pdf-purchase="${pu.id}" title="Télécharger le PDF">PDF</button>`}${canEdit ? `<button type="button" class="btn btn-sm icon-btn" data-edit-purchase="${pu.id}" title="Modifier l’achat" aria-label="Modifier l’achat">${ICONS.edit}</button>` : ''}${canDelete ? `<button type="button" class="btn btn-sm icon-btn btn-danger" data-delete-purchase="${pu.id}" title="Supprimer l’achat" aria-label="Supprimer l’achat">${ICONS.trash}</button>` : ''}</td>
            </tr>`;
          }).join('')}
        </tbody>
      </table>`}
        ...
        <div id="purchaseLines">
          ${lines.map(l => `
            <div class="purchase-line-grid">
              ...
              <div class="field"><label>Produit</label><select id="pl_product_${l.id}" data-purchase-product="${l.id}">
        ...
        <tbody>
          ${rows.map(r => {
            ...
            <tr>
              ...
              <td data-label="Stock actuel"><span class="pill ${status === 'ok' ? 'pill-ok' : status === 'warn' ? 'pill-warn' : 'pill-danger'}">${r.stockCurrent}</span></td>
              ${canViewCosts ? `<td data-label="Marge unitaire" class="price">${money(r.unitPrice - r.unitCost)}</td>` : ''}
...
function renderPOS() {
  ...
  const list = state.products.filter(p =>
    ...
    (!q || normalizeSearchText(p.name).includes(q) || normalizeSearchText(p.brand).includes(q) || normalizeSearchText(p.sku).includes(q))
  );
      ...
      <div class="pos-products">
        <div class="toolbar">
          <div style="display:flex;gap:6px;flex-wrap:wrap;">
            ${['Tous', ...getCategories()].map(c => `<button class="btn btn-sm ${cat === c ? 'btn-primary' : ''}" data-pos-cat="${esc(c)}">${esc(c)}</button>`).join('')}
          </div>
        </div>
        <div class="pos-grid">
          ${list.map(p => {
      ...
      <div class="pos-right">
        ...
        <div class="checkout-panel">
          ...
          <div class="paid-row">
            ...
            <input type="text" inputmode="decimal" autocomplete="off" data-decimal-input="1" id="posDiscount" value="${esc(state.posDiscount)}" placeholder="0">
          </div>
          ` : ''}
          ...
          <div class="paid-row" style="margin-bottom:0;">
            ...
            <input type="text" inputmode="decimal" autocomplete="off" data-decimal-input="1" id="posPaidAmount" value="${esc(paidDisplay)}">
          </div>
...
function buildOperationsReportData() {
  ...
  const sales = state.sales.filter(inRange).sort((a, b) => new Date(b.date) - new Date(a.date));
  const purchases = state.purchases.filter(inRange).sort((a, b) => new Date(b.date) - new Date(a.date));
  ...
  sales.forEach(sale => {
    ...
    const amount = Number(sale.total); const netAmount = Number.isFinite(amount) ? amount : saleNet;
    salesAmount += netAmount; salesGross += saleGross; salesDiscount += discount; salesCost += saleCost; salesMargin += netAmount - saleCost; salesUnits += saleUnits;
    rows.push({ date: sale.date, type: 'VENTE', ref: `Bon #${sale.number || sale.id || '—'}`, description: items.map(item => `${item.qty}× ${item.name || 'Produit'}`).join(', ') || 'Vente', party: client?.name || 'Client de passage', units: saleUnits, amount: netAmount, incoming: netAmount, outgoing: 0, impact: netAmount });
  });
...
function renderMarginReports() {
  const f = state.reportFilters || (state.reportFilters = { from: '', to: '', sellerId: '', productId: '' });
          ...
          <tbody>
            ${sales.map(s => {
              ...
              <tr ${isInvoiced ? 'style="opacity:.6;"' : ''}>
                ...
                <td data-label=""><button class="btn btn-sm" data-print-sale="${s.id}">${ICONS.printer}Bon</button><button class="btn btn-sm" data-print-delivery="${s.id}" title="Bon de livraison">${ICONS.printer}BL</button><button class="btn btn-sm" data-pdf-delivery="${s.id}" title="Télécharger le PDF">PDF</button><button class="btn btn-sm" data-share-delivery-whatsapp="${s.id}" title="Envoyer par WhatsApp">WA</button><button class="btn btn-sm" data-share-delivery-email="${s.id}" title="Envoyer par e-mail">E-mail</button></td>
              </tr>
            `; }).join('')}
          </tbody>
        </table>
        ...
        <tbody>
          ${sorted.map(s => {
            ...
            <tr>
              ...
              <td data-label="Total" class="price">${money(s.total)}</td>
              <td data-label=""><button class="btn btn-sm" data-print-sale="${s.id}">${ICONS.printer}</button><button class="btn btn-sm" data-print-delivery="${s.id}" title="Bon de livraison">${ICONS.printer}BL</button><button class="btn btn-sm" data-pdf-delivery="${s.id}" title="Télécharger le PDF">PDF</button><button class="btn btn-sm" data-share-delivery-whatsapp="${s.id}" title="Envoyer par WhatsApp">WA</button><button class="btn btn-sm" data-share-delivery-email="${s.id}" title="Envoyer par e-mail">E-mail</button></td>
            </tr>`;
          }).join('')}
        </tbody>
      </table>`}
...
async function recordGeneratedDocument(kind, record, channel='print') {
  const entry={id:uid('doc'),kind,recordId:record.id,number:record.number||record.id,date:new Date().toISOString(),channel,clientId:record.clientId||null};
...
function downloadBonPDF(kind, record) {
  ...
  const lines = [isPurchase ? `N° : ${record.id}` : `N° : ${record.number || record.id}`, `Date : ${fmtDateTime(record.date)}`, `${isPurchase ? 'Fournisseur' : 'Client'} : ${(supplier?.name || client?.name || 'Non spécifié')}`, ''];
  ...
  doc.save(`${isPurchase?'bon-achat':'bon-livraison'}-${record.number||record.id}.pdf`);
}
function shareBon(kind, record, channel) {
  ...
  const text=`Bonjour${recipient?.name ? ' '+recipient.name : ''}, veuillez trouver votre ${label} ${record.number||record.id} de ${getAppName()}. Le PDF a été téléchargé et peut être joint à ce message.`;
  ...
  else { const email=recipient?.email||''; if(!email){toast('Aucun e-mail enregistré pour ce client.');return;} window.location.href=`mailto:${encodeURIComponent(email)}?subject=${encodeURIComponent(label+' '+(record.number||record.id))}&body=${encodeURIComponent(text)}`; }
  downloadBonPDF(kind,record);
}
function buildPurchaseOrderHTML(purchase) { const supplier=state.suppliers.find(s=>s.id===purchase.supplierId); return `<div class="invoice-doc">${companyHeaderHTML()}<h2>Bon d’achat — ${esc(getAppName())}</h2><p>N° ${esc(purchase.id)} · Date : ${fmtDateTime(purchase.date)}</p><p>Fournisseur : <strong>${esc(supplier?.name || 'Non spécifié')}</strong></p><table><thead><tr><th>Produit</th><th>Code P</th><th>Qté</th><th>Prix achat</th><th>Total</th></tr></thead><tbody>${(purchase.items||[]).map(i=>`<tr><td>${esc(i.name)}</td><td>${esc(i.sku||'—')}</td><td>${i.qty}</td><td>${money(i.unitCost)} DH</td><td>${money((Number(i.qty)||0)*(Number(i.unitCost)||0))} DH</td></tr>`).join('')}</tbody></table><p style="text-align:right"><strong>Total : ${money(purchase.total)} DH</strong></p></div>`; }
function buildDeliveryNoteHTML(sale) { const client=state.clients.find(c=>c.id===sale.clientId); return `<div class="invoice-doc">${companyHeaderHTML()}<h2>Bon de livraison — ${esc(getAppName())}</h2><p>N° ${sale.number||sale.id} · Date : ${fmtDateTime(sale.date)}</p><p>Client : <strong>${esc(client?.name || 'Client comptant')}</strong></p><table><thead><tr><th>Produit</th><th>Code P</th><th>Qté</th></tr></thead><tbody>${(sale.items||[]).map(i=>`<tr><td>${esc(i.name)}</td><td>${esc(i.sku||'—')}</td><td>${i.qty}</td></tr>`).join('')}</tbody></table><p style="text-align:right"><strong>Total articles : ${(sale.items||[]).reduce((n,i)=>n+(Number(i.qty)||0),0)}</strong></p></div>`; }
/* ---------------- Settings ---------------- */
...
function exportPurchasesXLSX() {
  ...
  state.purchases.forEach(pu => (pu.items || []).forEach(it => { const p = state.products.find(x => x.id === it.productId); const sup = state.suppliers.find(x => x.id === pu.supplierId);
    rows.push({ 'ID achat': pu.id, Date: fmtDate(pu.date), 'Code fournisseur': sup ? sup.code : '', Fournisseur: sup ? sup.name : '', SKU: p ? p.sku : '', 'Nom du produit': it.name || (p && p.name) || '', Quantité: Number(it.qty) || 0, "Prix d'achat (DH)": Number(it.unitCost) || 0, 'Prix de vente (DH)': Number(it.unitPrice) || 0, Note: pu.note || '' });
  }));
...
async function importPurchasesXLSX(file) {
  try {
    ...
    const groups = new Map(); rows.forEach(r => { const key = String(r['ID achat'] || `import-${r.Date || 'now'}`).trim(); if (!groups.has(key)) groups.set(key, []); groups.get(key).push(r); });
    const purchases = [...groups.entries()].map(([key, group]) => { const first = group[0]; const items = group.map(r => { const p = findProductBySku(r.SKU); return { productId: p.id, name: p.name, qty: excelNumber(r.Quantité), unitCost: excelNumber(r["Prix d'achat (DH)"], 0), unitPrice: excelNumber(r['Prix de vente (DH)'], productSellPrice(p.id) || 0), qtyRemaining: excelNumber(r.Quantité) }; });
      return { id: uid('pu'), date: excelDate(first.Date), supplierId: findSupplierByImport(first['Code fournisseur'], first.Fournisseur)?.id || null, items, total: items.reduce((s, x) => s + x.qty * x.unitCost, 0), note: first.Note || `Import Excel (${key})` }; });
...
async function restoreFromFile(file) {
  ...
  try {
    ...
    previous = {
      ...
      purchases: state.purchases, invoices: state.invoices,
    };
    const next = {
    ...
    if (data.branding && typeof data.branding === 'object') state.branding = { ...state.branding, ...data.branding };
    ...
    if (settings.reportFilters) state.reportFilters = { ...state.reportFilters, ...settings.reportFilters };
    ...
    const results = await Promise.all([
      ...
      saveOplog(), saveSuppliers(), savePurchases(), saveInvoices(), safeSet('shop:theme-pref', state.theme, false), safeSet('shop:branding', state.branding, false),
    ]);
    ...
    if (previous) {
      ...
      try {
        await Promise.all([saveProducts(), saveCategories(), saveClients(), saveSales(), saveUsers(), savePayments(), saveOplog(), saveSuppliers(), savePurchases(), saveInvoices()]);
...
function bindModuleExcelEvents() {
  const bind = (buttonId, exporter) => {
    ...
    if (button) button.addEventListener('click', exporter);
  };
  ...
  const importOnly = (buttonId, inputId, importer) => {
    ...
    if (button && input) { button.addEventListener('click', () => { input.value = ''; input.click(); }); input.addEventListener('change', async e => { const file = e.target.files && e.target.files[0]; if (file) await importer(file); }); }
  };
...
function bindEvents() {
  ...
  document.querySelectorAll('[data-sort-target]').forEach(el => el.addEventListener('click', () => {
    ...
    const stateKey = target === 'products' ? 'productSort' : 'stockSort';
  ...
  if (scanProductCodeBtn) scanProductCodeBtn.addEventListener('click', () => openScanner('products'));
  const supplierSearch=document.getElementById('supplierSearch'); if(supplierSearch) supplierSearch.addEventListener('input',e=>{state.supplierSearch=e.target.value;refreshContent();});
  const purchaseSearch=document.getElementById('purchaseSearch'); if(purchaseSearch) purchaseSearch.addEventListener('input',e=>{state.purchaseSearch=e.target.value;refreshContent();});
  const salesSearch=document.getElementById('salesSearch'); if(salesSearch) salesSearch.addEventListener('input',e=>{state.salesSearch=e.target.value;refreshContent();});
  ...
  if (stockSearch) stockSearch.addEventListener('input', (e) => { state.stockSearch = e.target.value; refreshContent(); });
  ...
  if (inlineSaveCategoryBtn) inlineSaveCategoryBtn.addEventListener('click', async () => {
    ...
    if (select) { const option = new Option(name, name, true, true); select.add(option); select.value = name; }
...
document.querySelectorAll('[data-print-sale]').forEach(el => el.addEventListener('click', () => {
  ...
  if (mr_clear) mr_clear.addEventListener('click', () => { state.reportFilters = { from: '', to: '', sellerId: '', productId: '' }; refreshContent(); });
  ...
  function syncPurchaseDraftFromDOM() {
    ...
    state.purchaseDraftLines.forEach(line => {
      ...
      if (productEl) line.productId = productEl.value;
      if (!line.productId && line.sku) { const match = findProductBySku(line.sku); if (match) line.productId = match.id; }
      if (line.productId) { const product = state.products.find(p => p.id === line.productId); if (product && !line.sku) line.sku = product.sku || ''; }
  ...
  if (addPurchaseBtn) addPurchaseBtn.addEventListener('click', () => {
    ...
    state.purchaseDraftSupplierId = ''; state.purchaseDraftDate = localDateInputValue(); state.purchaseDraftTime = localTimeInputValue(); state.purchaseDraftNote = ''; state.purchaseScanLineId = null;
  ...
  document.querySelectorAll('[data-pdf-purchase]').forEach(el => el.addEventListener('click', () => { const purchase=state.purchases.find(x=>x.id===el.dataset.pdfPurchase); if (purchase) downloadBonPDF('purchase',purchase); }));
  document.querySelectorAll('[data-pdf-purchase]').forEach(el => el.addEventListener('click', () => { const purchase=state.purchases.find(x=>x.id===el.dataset.pdfPurchase); if (purchase) recordGeneratedDocument('purchase',purchase,'PDF'); }));
  document.querySelectorAll('[data-edit-purchase]').forEach(el => el.addEventListener('click', () => {
    ...
    state.purchaseDraftLines = (purchase.items || []).map((item, index) => { const product = state.products.find(p => p.id === item.productId); return { id: item.id || `${purchase.id}_${index}`, productId: item.productId || '', sku: item.sku || product?.sku || '', qty: item.qty ?? '', unitCost: item.unitCost ?? '', unitPrice: item.unitPrice ?? '' }; });
  ...
  if (cancelPurchaseModal) cancelPurchaseModal.addEventListener('click', () => { state.editingPurchase = null; state.purchaseDraftLines = null; state.purchaseDraftSupplierId = ''; state.purchaseDraftDate = ''; state.purchaseDraftTime = ''; state.purchaseDraftNote = ''; state.purchaseScanLineId = null; render(); });
  ...
  if (addPurchaseLine) addPurchaseLine.addEventListener('click', () => {
    ...
    state.purchaseDraftLines.push({ id: uid('pl'), productId: '', sku: '', qty: '', unitCost: '', unitPrice: '' });
    render();
  });
  ...
  document.querySelectorAll('[data-purchase-sku]').forEach(el => el.addEventListener('input', () => {
    ...
    line.sku = el.value.trim(); const product = findProductBySku(line.sku); if (product) { line.productId = product.id; const select = document.getElementById(`pl_product_${line.id}`); if (select) select.value = product.id; }
  }));
  ...
  if (savePurchaseModal) savePurchaseModal.addEventListener('click', async () => {
    ...
    for (const l of state.purchaseDraftLines) {
      ...
      if (!p) continue;
      items.push({ productId, name: p.name, qty, unitCost, unitPrice, qtyRemaining: qty });
    }
    ...
    const existingPurchase = state.editingPurchase && state.editingPurchase !== 'new' ? state.editingPurchase : null;
    const nextPurchase = { id: existingPurchase?.id || uid('pu'), date: localIsoFromParts(date, time), supplierId, items, total, note };
    ...
    state.editingPurchase = null; state.purchaseDraftLines = null; state.purchaseDraftSupplierId = ''; state.purchaseDraftDate = ''; state.purchaseDraftTime = ''; state.purchaseDraftNote = ''; state.purchaseScanLineId = null;
