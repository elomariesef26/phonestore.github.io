...
async function loadAll() {
  try {
    const [p, cat, c, s, u, pay, log, t, brandingStored, sess, sup, pur, inv, generatedDocs, reportFiltersStored, replenishmentBonsStored, replenishmentConsumedStored, replenishmentConsumedProductsStored, replenishmentSalesFiltersStored] = await Promise.all([
...
function renderPurchaseModal() {
  ...
  <div class="modal-overlay" role="dialog" aria-modal="true" id="purchaseModalOverlay">
    <div class="modal modal-wide purchase-modal">
      <h2>${isReplenishmentDraft ? (state.editingReplenishmentBon ? 'Modifier le bon d’achat' : 'Nouveau bon d’achat') : (existingPurchase ? 'Modifier l’achat' : 'Nouvel achat')}</h2>
      <div class="field-row">
      ...
      <div class="modal-actions">
        ...
        <button class="btn btn-primary" id="savePurchaseModal">${isReplenishmentDraft ? 'Enregistrer le bon d’achat' : "Enregistrer l'achat"}</button>
      </div>
    </div>
  </div>`;
}
...
function isValidBackup(data) {
  ...
  const optional = ['categories', 'sales', 'users', 'payments', 'oplog', 'suppliers', 'purchases', 'replenishmentBons', 'replenishmentConsumedSaleIds', 'replenishmentConsumedProductIds', 'invoices'];
...
async function restoreFromFile(file) {
  ...
  try {
    ...
    const restoreKeys = ['shop:products', 'shop:categories', 'shop:clients', 'shop:sales', 'shop:users', 'shop:payments', 'shop:oplog', 'shop:suppliers', 'shop:purchases', 'shop:replenishment-bons', 'shop:replenishment-consumed-sales', 'shop:replenishment-consumed-products', 'shop:invoices', 'shop:generated-documents', 'shop:report-filters', 'shop:branding', 'shop:theme-pref', 'shop:session'];
    ...
    if (previous) {
      ...
      try {
        await Promise.all([saveProducts(), saveCategories(), saveClients(), saveSales(), saveUsers(), savePayments(), saveOplog(), saveSuppliers(), savePurchases(), saveReplenishmentBons(), saveReplenishmentConsumedSales(), safeSet('shop:replenishment-consumed-products', JSON.stringify(state.replenishmentConsumedProductIds || []), true), saveInvoices(), safeSet('shop:generated-documents', JSON.stringify(state.generatedDocuments || []), true), safeSet('shop:report-filters', JSON.stringify(state.reportFilters || {}), false), safeSet('shop:theme-pref', state.theme, false), safeSet('shop:branding', JSON.stringify(state.branding || {}), false)]);
...
document.querySelectorAll('[data-print-sale]').forEach(el => el.addEventListener('click', () => {
  ...
  document.querySelectorAll('[data-dashboard-chart]').forEach(el => el.addEventListener('click', () => { state.dashboardChartMetric = el.dataset.dashboardChart; refreshContent(); }));
  ...
  const generatePurchaseFromSalesBtn = document.getElementById('generatePurchaseFromSalesBtn');
  bindOnce(generatePurchaseFromSalesBtn, 'click', openPurchaseFromSales);
  ...
  if (savePurchaseModal) savePurchaseModal.addEventListener('click', async () => {
    ...
    if (state.purchaseDraftMode === 'replenishment') {
      ...
      toast('Bon d’achat enregistré dans Réapprovisionnements'); render(); return;
    }
    const existingPurchase = state.editingPurchase && state.editingPurchase !== 'new' ? state.editingPurchase : null;
